    ...
    <div v-if="sla > 0">
      ...
      <v-row v-if="!isRoomRiskManager" align="center" no-gutters>
        ...
        <div v-if="showDetail">
          <v-col cols="6">
            <v-card-subtitle
              >Connection Invitation for {{ myId }}</v-card-subtitle
            >
            <v-img
              id="qrRoom"
              class="white--text align-end"
...
export default {
  computed: {
    ...
    selectedConnection() {
      if (this.selected.length) {
        if (this.selected.length > 1) {
          return 'Multiple';
        } else return this.selected[0].myId;
      }
      return '';
    },
    ...
    myId: {
      get() {
        return this.state?.myId;
      },
      set(newVal) {
        State.changeMyId(newVal).then(state => {
          // ORM returns an array of State objects
          this.state = state[0];
        });
      }
    ...
    isRoom() {
      return this.state?.roomId == this.state?.myId;
    }
  },
  watch: {
    overlay(val) {
  ...
  methods: {
    ...
    onGetCovidAlerts() {
      ...
      axios(
        `/messages/connection/?myId=${this.roomName}&field=ACCESSTOK_SOTERIA_LAB`
      ).then(s => {
    ...
    async onGetRoomWarnings() {
      ...
      this.overlay = true;
      axios(
        `/messages/connection/?myId=${this.managedRoom}&field=ACCESSTOK_SOTERIA_LAB`
      ).then(s => {
    ...
    // Because the myId is a guid (because its On their phone, we cache the roomName and message type
    onRoomSignIn() {
      if (!this.myId || !this.roomName) {
        alert(
          'Houston, we have a problem. onRoomSignIn() needs values for both this.myId and this.roomName.'
        );
        return;
      }
      let text = JSON.stringify({
      ...
      const myId = this.roomName;
      axios({ url: '/messages', data: payload, method: 'POST' })
        .then(() => {
          console.log(`Signed into: ${myId}`);
          DataRepository.connect({
            myId: myId,
            type: 'check-in'
          });
    ...
    onWarnRooms() {
      ...
      console.warn(this.myId, this.selectedConnection);
      let id = this.myId;
