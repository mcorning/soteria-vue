...
export default {
  computed: {
    ...
    roomSet() {
      let s = new Set(this.connections.map(v => v.myId));
      return Array.from(s);
    },
    messageSet() {
  ...
  methods: {
    ...
    // Because a phone myId is a guid, we cache the roomName and message type
    onRoomSignIn() {
    ...
    async alertRooms() {
      ...
      this.roomSet.forEach(async v => {
        let payload = {
          myId: v,
          text: `ALERT: ${this.myId} is in quarantine.`
        };
    ...
    onGetCovidAlerts() {
      ...
      axios(
        `/messages/connection/?myId=${this.myId}&field=ACCESSTOK_SOTERIA_LAB`
      ).then(s => {
        this.alerts = s.data
    ...
    async makeConnection() {
      ...
      await axios('/connections/?name=' + this.myId).then(s => {
    ...
    async getState() {
      console.log('in VisitorCard0.getState()');
      let s = await DataRepository.getState();
      // console.log('state:', s);
      this.state = s;
      console.log('in VisitorCard0 this.state:', this.state);
  ...
  async created() {
    console.log('Entering created() in VisitorCard0: getting State');
    ...
    this.loading = false;
    this.overlay = false;
    console.log('Leaving created() in VisitorCard0');
  }
};
</script>
