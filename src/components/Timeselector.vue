<template>
  <div class="vtimeselector">
    ...
    <div class="vtimeselector__clear" @click="clearTime">
      <slot name="clear-ico"
        ><span class="vtimeselector__clear__ico">x</span></slot
      >
    </div>

    <div
      class="vtimeselector__box"
      :class="{ 'vtimeselector__box--is-closed': pickerState.isClosed }"
    >
      <ul
        class="vtimeselector__box__list vtimeselector__box__list--hours"
        v-if="displayHours"
      >
        ...
        <li
          ...
          :class="{
            'timeselector__box__item--is-selected':
              pickerState.selected.hour == hour,
            'timeselector__box__item--is-highlighted': getState(
              'hour',
              'highlight',
              hour
            ),
            'timeselector__box__item--is-disabled': getState(
              'hour',
              'disable',
              hour
            )
      ...
      <ul
        class="vtimeselector__box__list vtimeselector__box__list--minutes"
        v-if="displayMinutes"
      >
        ...
        <li
          ...
          :class="{
            'timeselector__box__item--is-selected':
              pickerState.selected.minute == minute,
            'timeselector__box__item--is-highlighted': getState(
              'minute',
              'highlight',
              minute
      ...
      <ul
        class="vtimeselector__box__list vtimeselector__box__list--seconds"
        v-if="displaySeconds"
      >
        <li class="vtimeselector__box__head"><slot name="seconds">ss</slot></li>
        <li
          class="vtimeselector__box__item vtimeselector__box__item--seconds"
          v-for="(second, index) in timeCount(interval.s)"
          :key="index"
          :class="{
            'timeselector__box__item--is-selected':
              pickerState.selected.second == second,
            'timeselector__box__item--is-highlighted': getState(
              'second',
              'highlight',
              second
      ...
      <ul
        class="vtimeselector__box__list vtimeselector__box__list--ampm"
        v-if="!h24"
      >
        <li class="vtimeselector__box__head">
          <slot name="ampm">AM / PM</slot>
        </li>
        <li
          class="vtimeselector__box__item vtimeselector__box__item--ampm"
          :class="{
            'timeselector__box__item--is-selected':
              pickerState.selected.ampm === 'AM'
          }"
          @click="selectTime('ampm', 'AM', $event)"
        >
          AM
        </li>
        <li
          ...
          :class="{
            'timeselector__box__item--is-selected':
              pickerState.selected.ampm === 'PM'
          }"
          @click="selectTime('ampm', 'PM', $event)"
        >
          PM
        </li>
      </ul>
...
export default {
  ...
  computed: {
    ...
    displayRules() {
      return {
        ...
        kk: () =>
          this.pad(
            this.picker.hour === 0 ? this.picker.hour + 1 : this.picker.hour,
            true
          ),
        k: () =>
          this.picker.hour === 0 ? this.picker.hour + 1 : this.picker.hour,
  ...
  methods: {
      ...
      if (type === 'displayFormat' && !this.displayFormat)
        return (
          ...
          (this.displayMinutes && this.displayHours
            ...
            : '') +
          (this.displaySeconds
            ? this.separator + this.pad(this.picker.second)
            : '') +
    ...
    timeCount(interval = 1, timeLength = 60) {
      return Array.apply(null, { length: timeLength })
        .map(Number.call, Number =>
          timeLength < 60 ? (!this.h24 ? Number + 1 : Number) : Number
        )
     ...
     * @param {String} type - kind of change (hour, minute, second, ampm)
     * // mpc added el
     * @param {Object} el - value of change
    ...
    togglePicker() {
      if (!this.disabled) {
         ...
         * @type {null}
         */
        this.pickerState.isClosed
          ? this.$emit('closed', this.$el)
    ...
    getState(type, state, time) {
      if (this[state][type.charAt(0)]) {
        ...
        const timeAskedList = this[state][type.charAt(0)].map(h => {
          ...
          return this.h24 === false && timeAction > 12
            ? timeAction - 12
            : timeAction;
        });
  ...
  watch: {
    value: function() {
      this.picker.hour = this.value
        ? this.utc
          ? this.value.getUTCHours()
          : this.value.getHours()
        : 0;
      this.picker.minute = this.value
        ? this.utc
          ? this.value.getUTCMinutes()
          : this.value.getMinutes()
        : 0;
      this.picker.second = this.value
        ? this.utc
          ? this.value.getUTCSeconds()
          : this.value.getSeconds()
        : 0;
      ...
      if (!this.pickerState.isPristine) {
        ...
        this.pickerState.selected.ampm = this.h24
          ? null
          : this.picker.hour > 12
          ? 'PM'
          : 'AM';
  ...
  created() {
    ...
    this.value !== null &&
      ['hour', 'minute', 'second'].forEach(
        type =>
          (this.pickerState.selected[type] =
            this.picker[type] && this.pad(this.picker[type]))
      );
    ...
    if (
      ...
      (this.disable.h && this.disable.h.length > 0)
    ) {
      console.warn(
        'You shouldn\'t use disable hour props or h24="false" with highlight props. It may cause AM-PM confusion or limitation due to multiple hours selected.'
      );
    }
